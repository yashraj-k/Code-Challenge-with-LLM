...
def reset_quota_if_needed(db: Session , quota :models.ChallengeQuota):
    ...
    if now - quota.last_reset_date > timedelta(hours=24):
        quota.remaining_quota = 5
        quota.last_reset_date = now
        db.commit()
        db.refresh(quota)
    return quota

def create_challenge(db: Session, difficulty:str , created_by:str ,title:str,
                     options:str, correct_answer_id:int , explanation:str):
...
def get_user_challenge(db: Session, user_id:int):
